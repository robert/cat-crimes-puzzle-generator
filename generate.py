...
CLUE_PRIORITIES = [Sleeping] + CLUES


def order_clues(clues: list[Clue]) -> list[Clue]:
    priority_dict = {cls: idx for idx, cls in enumerate(CLUE_PRIORITIES)}
    def priority_key(clue):
        return priority_dict.get(type(clue), len(CLUE_PRIORITIES))
...
# Example usage
# save_solution_img(cat_positions, 'shoes', '/path/to/save/dir', '/path/to/image/dir')
...
def generate_puzzle(clue_types: list, requirements: list[Callable]) -> dict:
    generator = CatCrimesPuzzleGenerator(clue_types=clue_types)
    while True:
...
new_puzzle = generate_trivial_challenge()
...
d = os.path.abspath(f'./outputs/{uuid}')
